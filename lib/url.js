...
Url.prototype.params = function (path) {
  ...
  for (var i = 1, len = m.length; i < len; ++i) {
    ...
    if (key) {
      params[key.name] = params[key.name] !== undefined ? params[key.name] : value;
    } else {
      params.push(value);
    }
  }
...
Url.prototype.resolve = function (params, options) {
  ...
  if (missingParams.length === 0) {
    ...
    throw new Error("Missing required parameters on path " + JSON.stringify(this._originalPath) +
      ". The missing params are: " + JSON.stringify(missingParams) +
      ". The params object passed in was: " + JSON.stringify(originalParams) + ".");
  } else {
    return null;
  }
};
